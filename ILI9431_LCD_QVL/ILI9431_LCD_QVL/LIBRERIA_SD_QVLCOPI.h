...
uint32_t BS_VolID=0,BPB_RootClu=0;
...
unsigned int SEARCH_CLUSTER_FILE(uint16_t SIZE_SECTOR,const char YE[]){			READ_MBR(512);					char NOMBRE_DEL_ARCHIVO[11];
for(int i=0;i!=11;i++){
NOMBRE_DEL_ARCHIVO[i]=0x20;}
int i=0,EXTENCION_DEL_FILE=0;
while(i!=8){
if(YE[i]=='.'){EXTENCION_DEL_FILE=i;i=8;}
else{NOMBRE_DEL_ARCHIVO[i]=YE[i];
i++;}}
for(int i=8;i!=11;i++){EXTENCION_DEL_FILE++;
NOMBRE_DEL_ARCHIVO[i]=YE[EXTENCION_DEL_FILE];}
...
// if(BPB_FATz16 != 0){FATSz = BPB_FATz16;}
// else {FATSz = BPB_FATz32;}
// if(TYPPE_FAT == 16) {
// FATOffset=N*2; }
// else{
// if (FATType == FAT32){FATOffset = N * 4;}}
//ThisFATSecNum = BPB_RsvdSecCnt +(FATOffset / BPB_BytsPerSec); ThisFATEntOffset = REM(FATOffset / BPB_BytsPerSec);
...
if(BS_VolID==0xf8){}}
	...
	if((i==44)){
		...
		BPB_RootClu=(BPB_RootClu<<8)|READ_DATA[luis];}
		if(BPB_RootClu==0xf8){}}
...
void _MEMORY_SD_INFO(){
// LCD_PRINT("FAT:",13,0);CONVER_INT_CHAR(TYPPE_FAT,17,0);LCD_PRINT("]",19,0);
// LCD_PRINT("[#Sec*Clus:",0,0);CONVER_INT_CHAR(BPB_SecPerClus,11,0);
// LCD_PRINT("#Clusters:",0,1);CONVER_INT_CHAR(CountofClusters/1000,10,1);LCD_PRINT("[Mil]",15,1);
// LCD_PRINT("#FATs:",0,2);CONVER_INT_CHAR(BPB_NumFATs,6,2);
// LCD_PRINT("VOLUMEN:",0,3);CONVER_INT_CHAR(((DskSize*512)/1024000),8,3);LCD_PRINT("[MB]",16,3);
// PAUSE_FLUJO(20000,1,0,1);
// LIMPIAR_PANTALLA();
// LCD_PRINT("#SECTORS:",0,0);CONVER_INT_CHAR(((float)DskSize/1000),9,0);LCD_PRINT("[Mil]",15,0);
// LCD_PRINT("#Sec_D&F:",0,1);CONVER_INT_CHAR((float)FAT_AND_DATA_REGION/1000,9,1);LCD_PRINT("[Mil]",15,1);
// LCD_PRINT("#MBR:",0,2);CONVER_INT_CHAR(F_MBR,5,2);LCD_PRINT("[DEC]",15,2);
// LCD_PRINT("#SecReserv:",0,3);CONVER_INT_CHAR(BPB_RsvdSecCnt,11,3);
// PAUSE_FLUJO(20000,1,0,1);
// LIMPIAR_PANTALLA();
// LCD_PRINT("#PRIMER SECTOR DATOS",0,0);
// CONVER_INT_CHAR(FirstDataSector,0,1);
// LCD_PRINT("#PRIMER SEC DIR RAIZ",0,2);
// CONVER_INT_CHAR(FirstRootDirSecNum,0,3);
// PAUSE_FLUJO(20000,1,0,1);
// LIMPIAR_PANTALLA();
// LCD_PRINT("#Bytes Por Sector",0,0);
...
// LIMPIAR_PANTALLA();
// LCD_PRINT_NUMERO("BUSCANDO",0,0);
// LCD_PRINT_NUMERO("ARCHIVO:",0,1);
// PAUSE_FLUJO(15000,1,0,1);
// LIMPIAR_PANTALLA();
...
void READ_MEMORY_SD_INFO_FILE(char FILE[]){
uint16_t COMPROVATE;
COMPROVATE=SEARCH_CLUSTER_FILE(512,FILE);



if (COMPROVATE!=0){
LCD_PRINT_NUMERO("ARCHIVO ENCONTRADO:",0,0);
LCD_PRINT_NUMERO("TAMANO:",0,1);
CONVER_INT_CHAR(PESO_FILE,7,1);
LCD_PRINT("[KB]",16,1);}
...
case 0:
// LIMPIAR_PANTALLA();
// LCD_PRINT_NUMERO("ENCONTRADO:",0,1);
// CONVER_INT_CHAR(COMPROVATE,0,2);
break;}

PAUSE_FLUJO(45000,1,0,1);
LIMPIAR_PANTALLA();
}
